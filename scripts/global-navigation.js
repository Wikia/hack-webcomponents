...
customElements.define('global-navigation', class extends skate.Component {
	...
	logo(model) {
		...
		return <a class="wds-global-navigation__logo" href={model.module.main.href}>
	...
	fandomOverviewLinks(model) {
		if (!model || !model.links) {
			return;
		}

		return model.links.map((link) => this.linkBranded(link));
	}
	...
	search(model) {
		return <form class="wds-global-navigation__search" action={model.search.module.results.url}>
	...
	// TODO use model
	startWiki(/*model*/) {
		return <div class="wds-global-navigation__start-a-wiki">
